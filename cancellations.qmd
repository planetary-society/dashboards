...
---
title: NASA Contract Cancellations
format:
  dashboard:
    description-meta: "Daily updated dashboard tracking NASA grants and contracts terminated by the Trump Admininistration."
...
```{python}
#| title: "Impact by Business Type"
#| component: card
#| echo: false

# --- This chunk generates and displays the Altair histogram ---
# It uses the hist_data_series prepared in the setup chunk using the keyword method.

import altair as alt
import pandas as pd # Ensure pandas is available here if needed, though hist_data_series should be passed

# Ensure hist_data_series exists and is not empty
if 'hist_data_series' in locals() and not hist_data_series.empty:
    # Define colors from CSS
    planetary_blue = '#037cc2'

    # Convert the series to a DataFrame suitable for Altair
    hist_df = hist_data_series.reset_index().rename(columns={"index": "Category", 0: "Count"})

    # Create the Altair chart
    chart = alt.Chart(hist_df).mark_bar(color=planetary_blue).encode(
        x=alt.X('Count', axis=alt.Axis(title='Number of Cancelled Contracts')),
        y=alt.Y('Category', sort='-x', axis=alt.Axis(title='Business Category (Keyword Grouped)')), # Updated axis title
        tooltip=[
            alt.Tooltip('Category', title='Business Type'),
            alt.Tooltip('Count', title='Contracts Cancelled')
            ]
    ).properties(
        title='Cancelled Contracts by Simplified Business Category' # Updated title
    ).interactive()

    # Display the chart (Quarto will render this object)
    chart
else:
    # Display a message if data is not available
    print("Histogram data not available.")
```
