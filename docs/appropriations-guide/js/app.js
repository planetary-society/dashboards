...
class AppropriationsGuide {
        ...
        const handleClick = (e) => {
            ...
            if (awardsToggleBtn) {
                ...
                if (awardList) {
                    ...
                    awardsToggleBtn.textContent = isExpanded ? 'Hide active awards' : 'Show active awards';
                }
            }
            ...
            // Walkthrough button
            if (e.target.closest('.walkthrough-trigger-btn')) {
            ...
            // Dismiss button (tips callout, ai disclaimer)
            const dismissBtn = e.target.closest('.dismiss-btn');
            if (dismissBtn) {
                const panel = dismissBtn.closest('.tips-callout, .ai-disclaimer');
                if (panel) {
                    panel.style.transition = 'opacity 0.2s ease, margin-bottom 0.2s ease, max-height 0.25s ease';
            ...
            if (copyBtn) {
                const isInline = copyBtn.classList.contains('copy-btn--inline');
                const textToCopy = 'copyText' in copyBtn.dataset
        ...
        const awardRows = allAwards.map(award => {
            ...
            return `<div class="award-row">
                <div class="award-row-header">
                    ...
                    <span class="award-amount">${formatCurrency(award.award_amount)}<span class="award-amount-label">(potential value)</span></span>
                </div>
    ...
    renderFormWalkthrough(guideData, isCustom = false) {
        ...
        this.guideUrlType = fa.url_type || null;

        let html = '<div class="form-walkthrough">';

        if (isCustom) {
            const sourceDesc = this.getSourceDescription(fa.url_type);
            html += `<div class="ai-disclaimer">
                <i class="bi bi-stars"></i>
                <div>
                    <strong>AI-generated guide</strong> &mdash; Beta feature. ${sourceDesc} Review all content carefully and rewrite in your own voice before submitting.
                </div>
                <button class="dismiss-btn" aria-label="Dismiss"><i class="bi bi-x-circle-fill"></i></button>
            </div>`;

            // Source badge for PDF-extracted forms
            if (fa.url_type === 'pdf') {
                const formTitle = fa.form_header_text ? escapeHtml(fa.form_header_text) : 'Form';
                html += `<div class="source-badge">
                    <i class="bi bi-file-earmark-pdf"></i>
                    <span>Source: <strong>${formTitle}</strong> (PDF) &mdash; field names and structure were extracted by AI and may not exactly match the original document</span>
                </div>`;
            }
        }
        ...
        html += `<div class="tips-callout">
            ...
            <ul class="tips-list">
                <li><strong>Write in your own voice.</strong> Don't just copy/paste our suggestions. A personal message stands out. </li>
                <li><strong>Lead with local impact.</strong> Focus on how NASA Science funding benefits your community through investments in local businesses and research institutions.</li>
                <li><strong>Keep it concise.</strong> A few focused, compelling sentences are justification enough.</li>
            </ul>
            <button class="dismiss-btn" aria-label="Dismiss"><i class="bi bi-x-circle-fill"></i></button>
        </div>`;
        ...
        sections.forEach((section, sIdx) => {
            ...
            const needsReview = isCustom ? fields.filter(f => !f.is_constituent_info && f.draft_value && (f.confidence === 'low' || f.confidence === 'medium')).length : 0;
            if (fields.length > 0) {
                let summaryParts = [];
...
[Your Phone Number]`;
    ...
    buildSectionNav(sections) {
        ...
        nav.innerHTML = `<div class="section-nav-inner"><div class="section-progress" role="navigation" aria-label="Form sections">${steps}</div><button class="walkthrough-trigger-btn" aria-label="Start guided walkthrough"><i class="bi bi-map"></i> Walkthrough</button></div>`;
    ...
    createStickyBar() {
        ...
        if (spending) {
            ...
            spendingHtml += `<span class="sticky-awards">${totalAwards} awards worth ${totalValue} during this period</span>`;
        }
    ...
    initWalkthrough() {
        const seen = localStorage.getItem('appr-guide-walkthrough-seen');
        if (!seen) {
            setTimeout(() => this.startWalkthrough(), 800);
        }
    }
    ...
    startWalkthrough() {
        ...
        const hasConfidence = !!document.querySelector('.confidence-display');
        const step4 = hasConfidence
            ? {
            ...
            : {
                ...
                popover: {
                    ...
                    description: 'Each field box tells you exactly what to enter. Color-coded labels help: <strong>TYPE</strong> (blue) = type in a value, <strong>COPY</strong> (green) = paste the suggested text directly, <strong>DROPDOWN</strong> (purple) = select from a menu, <strong>PERSONAL</strong> (teal) = fill in with your own constituent info.',
                    side: 'top',
                    align: 'start'
                }
            };

        const spendingSlot = document.getElementById('spending-details-slot');
        const hasSpendingData = !!(spendingSlot && spendingSlot.children.length > 0);

        const spendingStep = hasSpendingData ? [{
            element: '#spending-details-slot',
            popover: {
                title: 'NASA Funding in Their District',
                description: 'Individual award amounts shown as <strong>(potential value)</strong> are the total contract or grant ceiling over its entire lifetime — not what\'s spent per year. The <strong>FY annual totals</strong> (e.g., FY2023: $2.7M) show obligations: money actually committed that fiscal year. These numbers can be very different.',
                side: 'bottom',
                align: 'start'
            }
        }] : [];
        ...
        const driverObj = driverFn({
            ...
            smoothScroll: true,
            onDestroyStarted: () => {
                localStorage.setItem('appr-guide-walkthrough-seen', '1');
                driverObj.destroy();
            },
            steps: [
                {
    ...
    getSourceDescription(urlType) {
        switch (urlType) {
            case 'pdf':
                return 'Field labels and suggested responses were extracted by AI from a <strong>PDF form</strong> and may contain errors.';
            case 'google_forms':
                return 'Field labels and suggested responses were generated by AI based on a Google Form.';
            case 'microsoft_forms':
                return 'Field labels and suggested responses were generated by AI based on a Microsoft Form.';
            default:
                return 'Field suggestions and draft responses below were generated by AI based on the official form.';
        }
    }
    ...
    renderField(field) {
        ...
        const isLowConfidence = this.isCustomGuide && confidence === 'low' && hasDraft && !isConstituent;
        ...
        // Confidence bars + info icon (only for AI-generated custom guides)
        if (this.isCustomGuide && hasDraft && !isConstituent && confidence) {
            ...
            let infoHtml = '';
            if (field.rationale) {
                const sourcePrefix = this.guideUrlType === 'pdf' ? 'Extracted from PDF by AI' : 'AI generated result';
                const rationaleText = `${sourcePrefix} with the following justification: ${escapeHtml(field.rationale)}`;
    ...
    getFieldInstruction(field) {
        ...
        switch (type) {
            case 'text':
                return `<div class="text-value-block">Enter: &ldquo;<strong>${escapeHtml(field.draft_value)}</strong>&rdquo;<button class="copy-btn" data-copy-text="${escapeHtml(field.draft_value)}" title="Copy to clipboard"><i class="bi bi-clipboard"></i> Copy</button></div>`;
            ...
            default:
                return `<div class="text-value-block">Type: &ldquo;<strong>${escapeHtml(field.draft_value)}</strong>&rdquo;<button class="copy-btn" data-copy-text="${escapeHtml(field.draft_value)}" title="Copy to clipboard"><i class="bi bi-clipboard"></i> Copy</button></div>`;
        }
    }
