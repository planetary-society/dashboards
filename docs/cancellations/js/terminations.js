 ...
 * Terminations Module
 *
 * Pure helpers over `terminations.csv`, the federal-record half of the
 * cancellations dashboard. No DOM, no fetch, no state beyond the shared
 * warn-once registry, so this module is safe to import from both the browser
 * dashboard and Node test runners.
 *
 * Every row is a termination action found in an award's USAspending
 * transaction history — either by its FPDS action code or by explicit
 * termination language in the transaction description. Five of the 177 rows
 * are disclosed *partials*: the award itself continues and only part of its
 * work was cut (`descoped`) or it was wound down administratively
 * (`closed_out`). Partials stay in the table with their own badge but are kept
 * out of the headline count and out of every dollar total, because counting a
 * descope as a cancelled award overstates the effect (one descope alone,
 * Lockheed's NNG09FA40C, carries $194M).
 */
 ...
 */
export function normalizeTerminations(rawRows) {
