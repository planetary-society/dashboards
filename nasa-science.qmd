  ...
  dashboard:
    ...
    include-in-header:
      - text: |
          ...
          <script>
          console.log('Dashboard scripts initializing...');

          // Logo fix - targeting the exact structure from your HTML
          function updateLogoLink() {
              // Target: <a href="#"><img src="img/TPS_Logo_3Stack-White.png" alt="" class="navbar-logo d-inline-block"></a>
              const logoAnchor = document.querySelector('a[href="#"] > img.navbar-logo');
              
              if (logoAnchor) {
                  const anchor = logoAnchor.parentElement;
                  console.log('✓ Found logo anchor:', anchor);
                  ...
                  console.log('✓ Logo link updated successfully!');
                  return true;
              }
              
              // Also check for the navbar title link if needed
              const titleLink = document.querySelector('.navbar-title-text a[href="#"]');
              if (titleLink) {
                  console.log('Found navbar title link - leaving as is for dashboard navigation');
              }
          ...
          function adjustMapBounds() {
              ...
              const mapContainers = document.querySelectorAll('.folium-map, [id^="map_"], .leaflet-container');
              
              mapContainers.forEach(container => {
                  try {
                      let map = null;
                      
                      // Check if map object exists on the element
                      if (container._leaflet_map) {
                          map = container._leaflet_map;
                      } else if (container.id && window[container.id]) {
                          map = window[container.id];
                      }
                      
                      if (map && typeof map.fitBounds === 'function') {
                          const bounds = L.latLngBounds(
                              [23.7, -122.5],  // Southwest
                              [46.7, -68.79]   // Northeast
                          );
                          
                          let options = {};
                          if (viewportWidth < 768) {
                              options = { padding: [10, 10], maxZoom: 6 };
                          } else if (viewportWidth < 1024) {
                              options = { padding: [20, 20], maxZoom: 7 };
                          } else {
                              options = { padding: [30, 30], maxZoom: 4 };
                          }
                      ...
                      console.warn('Error adjusting map:', e);
                  }
              });
          ...
          function initialize() {
              ...
              function tryFixes() {
                  ...
                  if (!logoFixed) {
                      logoFixed = updateLogoLink();
                  }
                  
                  if (!mapFixed) {
                      mapFixed = adjustMapBounds();
                  }
                  
                  if ((!logoFixed || !mapFixed) && attempts < maxAttempts) {
                      setTimeout(tryFixes, 500);
                  } else {
                      console.log(`Initialization complete after ${attempts} attempts. Logo: ${logoFixed ? '✓' : '✗'}, Maps: ${mapFixed ? '✓' : '✗'}`);
                  }
              }
          ...
          window.addEventListener('resize', () => {
              ...
              resizeTimeout = setTimeout(adjustMapBounds, 300);
          });

          // Debug helper
          window.dashboardDebug = {
              checkStatus: () => {
                  const logo = document.querySelector('a[href="#"] > img.navbar-logo');
                  const logoHref = logo ? logo.parentElement.href : 'Not found';
                  const maps = document.querySelectorAll('.leaflet-container').length;
                  
                  console.log('Debug Status:');
                  console.log('- Logo href:', logoHref);
                  console.log('- Leaflet loaded:', typeof L !== 'undefined');
                  console.log('- Map containers:', maps);
              }
          };
          </script>
---
