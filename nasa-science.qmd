  ...
  dashboard:
    ...
    include-in-header:
      - text: |
          ...
          <script>
            console.log('Dashboard scripts loading at:', new Date().toISOString());

            // Strategy 1: Direct attempt with multiple selectors
            function updateLogoLink() {
                const selectors = [
                    'a[href="#"] img.navbar-logo',
                    'a[href="#"].navbar-brand',
                    '.navbar-brand[href="#"]',
                    'a.navbar-brand:not([href*="planetary.org"])',
                    '.navbar a[href="#"]',
                    'nav a[href="#"]'
                ];
                ...
                for (const selector of selectors) {
                    ...
                    if (element) {
                        ...
                        if (anchor && !anchor.href.includes('planetary.org')) {
                            anchor.href = 'https://planetary.org';
                            anchor.target = '_blank';
                            anchor.title = 'The Planetary Society';
                            anchor.setAttribute('rel', 'noopener noreferrer');
                            console.log('✓ Logo link updated using selector:', selector);
                            return true;
                        }
                    }
                }
            ...
            function watchForLogo() {
                const observer = new MutationObserver((mutations, obs) => {
                    if (updateLogoLink()) {
                        obs.disconnect();
                        console.log('✓ Logo link updated via MutationObserver');
                    }
                });
            ...
            // Strategy 3: Multiple timed attempts
            function attemptLogoUpdate() {
                const intervals = [0, 100, 500, 1000, 2000, 3000, 5000];
                
                intervals.forEach(delay => {
                    setTimeout(() => {
                        if (updateLogoLink()) {
                            console.log(`✓ Logo updated after ${delay}ms`);
                        }
                    }, delay);
                });
            }
            ...
            if (document.readyState === 'loading') {
                document.addEventListener('DOMContentLoaded', () => {
                    console.log('DOM loaded, starting logo update attempts');
                    attemptLogoUpdate();
                    watchForLogo();
                });
            } else {
                console.log('DOM already loaded, starting logo update attempts');
                attemptLogoUpdate();
                watchForLogo();
            }
          </script>
                    ...
                    mapElements.forEach(function(mapElement) {
                        ...
                        if (mapObj) {
                            ...
                            if (viewportWidth < 768) {
                                ...
                                maxZoom = 3; // Default zoom of 4 on desktop
                            }
