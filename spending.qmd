      ...
      - text: |
          ...
          <script>
            document.addEventListener('DOMContentLoaded', function() {
                // Select the FIRST anchor tag that is a DIRECT CHILD of the container
                const logoLinkAnchor = document.querySelector('.navbar-brand-container > a');

                // Alternatively, you could select the image and then get its parent element:
                ...
                if (logoLinkAnchor) {
                    ...
                    // Optional: For accessibility and SEO, you might want to set the title too
                    // logoLinkAnchor.title = 'Visit The Planetary Society homepage';
                } else {
...
# Removed glob as it's not needed for a fixed CSV file path.
...
hover_info_map = {}
# Required columns for hover info: 'district', 'avg_obligations', 'number_of_awards', 'list_of_recipients'
# Note: CSV headers are 'district', 'AwardAmount_Avg' (now 'avg_obligations'),
# 'NumberOfAwards', 'Recipient_List_Short'. Adjusting to match new 'avg_obligations'.
if all(col in df_cleaned.columns for col in ['GEOID', 'district', 'avg_obligations', 'NumberOfAwards', 'Recipient_List_Short']):
    for index, row in df_cleaned.iterrows():
        ...
        num_awards_str = f"{row['NumberOfAwards']}"
        recipients_str = str(row['Recipient_List_Short']) if pd.notna(row['Recipient_List_Short']) else "N/A"
        
        hover_text = (
            f"<b>District: {row['district']}</b><br>" # Use 'district' from CSV
            f"Average Science Obligation (FY22-24): {avg_amount_str}<br>"
            f"Number of Awards: {num_awards_str}<br>"
            f"Key Recipients: {recipients_str}"
        )
        hover_info_map[geoid] = hover_text
else:
    print("Warning: One or more required columns for hover_info not found in df_cleaned after processing. Hover info will be basic.")
    # Fallback hover info if columns are missing, using the new map
    for geoid_key in district_avg_obligations_map.keys():
        avg_amount_str = f"${district_avg_obligations_map.get(geoid_key, 0):,.0f}"
        hover_info_map[geoid_key] = f"<b>Average Science Obligation (FY22-24): {avg_amount_str}</b><br>(Detailed info unavailable)"
...
if congressional_geojson_data:
    ...
    # For log scale, the legend might be tricky. A simpler linear legend might be more practical
    # or manually creating tick marks for log scale.
    # For now, let's use a linear colormap for the legend based on actual spending values.
    
    # Filter out zero/NaN for legend min/max if log scaling was aggressive
    # Use min_avg_obligation and max_avg_obligation for the legend scale
    legend_min_val = min_avg_obligation if min_avg_obligation > 0 else 0
    legend_max_val = max_avg_obligation if max_avg_obligation > 0 else 1

    # Create a Branca colormap for the legend
    legend_colormap = LinearColormap(
        colors=[COLOR_LOW_SPENDING, COLOR_HIGH_SPENDING],
        index=[legend_min_val, legend_max_val], # Scale based on actual min/max positive avg obligations
        vmin=legend_min_val,
        vmax=legend_max_val
    )
